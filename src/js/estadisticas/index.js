...
const chartEstados = new Chart(context, {
    type: 'polarArea',
    data: {
        labels: ['Personal Pendiente a Inicar Proceso', 'Personal Pendiente a Calificar Notas', 'Personal Pendiente a Revisaar Requisitos'],
        datasets: [{
            label: 'Estados',
            data: [],
            backgroundColor: getRandomColors(3), // Generamos 3 colores para las 3 barras
...
const getEstadisticas = async () => {
    const url = `API/estadisticas/grafica`;
    const config = {
        method: 'GET'
    };
    ...
    try {
        ...
        if (data) {
            ...
            updateChart([activosCount, inactivosCount, requisitosCount]);
        } else {
            Toast.fire({
